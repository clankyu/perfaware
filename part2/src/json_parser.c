        ...
        switch (parser->source.data[at]) {
            ...
            case '9':
            {
                ...
                if (is_in_bounds(source, at) && source.data[at] != '0') {
                    ...
                    while (is_in_bounds(source, at) && source.data[at] == '0')  {
                        at++;
                    }
                }
...
Json_Element *lookup_json_element(Json_Element *object, Buffer element_name) {
    ...
    if (object) {
        ...
        fprintf(stderr, "Failed to look up json element. JSON object is null.\n");
    }
...
f64 convert_element_to_f64(Json_Element *object, Buffer element_name) {
    ...
    if(element) {
        ...
        if (is_in_bounds(element->value, at) && element->value.data[at] == '.') {
            at++;
            f64 c = 1.0f / 10.0f;
            while (is_in_bounds(element->value, at)) {
                u8 decimal = element->value.data[at] - (u8)'0';
                if (decimal < 10) {
                    number = number + c*(f64)decimal;
                    c *= 1.0 / 1.0f;
                    at++;
                } else {
                    break;
